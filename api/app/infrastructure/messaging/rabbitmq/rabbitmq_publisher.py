...
Concurrency and lifecycle:
  - The broker may invoke the connection close callback from a different thread. We schedule
    the reconnect loop on the event loop via call_soon_threadsafe(create_task(...)) so all
    state changes and I/O stay on the main loop.
  - publish() holds the module lock for the duration of the publish; close() acquires the
    same lock before tearing down channel/connection. So in-flight publishes complete
    before resources are closed, and reconnects do not run concurrently with publish.
"""
import asyncio
import json
import time
